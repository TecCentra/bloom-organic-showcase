...
// //                             {/* Next-action button — only the valid next step is shown */}
// //                             {order.status === 'processing' && (
// //                               <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 text-blue-700 border-blue-200 hover:bg-blue-50"
// //                                 onClick={() => handleOrderAction(order.order_id, 'pack', 'Pack Order')}>
// //                                 <Package className="h-3.5 w-3.5" /> Pack
// //                               </Button>
// //                             )}
// //                             {order.status === 'ready_for_pickup' && (
// //                               <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 text-indigo-700 border-indigo-200 hover:bg-indigo-50"
// //                                 onClick={() => handleOrderAction(order.order_id, 'pickup', 'Mark Picked Up')}>
// //                                 <UserCheck className="h-3.5 w-3.5" /> Pickup
// //                               </Button>
// //                             )}
// //                             {order.status === 'picked_up_by_courier' && (
// //                               <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5 text-orange-700 border-orange-200 hover:bg-orange-50"
// //                                 onClick={() => handleOrderAction(order.order_id, 'out-for-delivery', 'Out for Delivery')}>
...
// //                                 onClick={() => handleOrderAction(order.order_id, 'deliver', 'Mark Delivered')}>
...
//             return (
//               <div className="space-y-6 overflow-y-auto flex-1 px-6 pb-4 pt-2">
...
//                 <div className="flex justify-end px-6 py-3 border-t border-gray-100 flex-shrink-0 bg-white">
//                   <Button variant="outline" onClick={() => setIsModalOpen(false)}>Close</Button>
//                 </div>
//               </div>
//             );
...
const AdminOrders: React.FC = () => {
  ...
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
  const [isModalOpen, setIsModalOpen]     = useState(false);
  // Inline confirm state — used inside the modal to avoid z-index stacking issues
  // with useMaterialConfirm which renders its own dialog on top of our modal
  const [inlineConfirm, setInlineConfirm]   = useState<{
    action: 'pack' | 'pickup' | 'out-for-delivery' | 'deliver' | 'assign-courier';
    label: string;
    orderId: string;
    courierData?: { courier_name: string; courier_tracking_number: string; delivery_notes?: string };
  } | null>(null);
  const [statusFilter, setStatusFilter]   = useState('all');
  const [orders, setOrders]               = useState<Order[]>([]);
  const [isLoading, setIsLoading]         = useState(true);
  ...
  const handleOrderAction = async (
    ...
    // When called from inside the modal, skip the external confirm dialog
    // (which causes z-index stacking issues) and use the inline confirm instead
    if (!skipConfirm) {
      const confirmed = await confirm({
        title: `Confirm: ${label}`,
        message: `Are you sure you want to ${label.toLowerCase()} this order?`,
  ...
  // Core assign-courier API call — called after inline confirm resolves
  const executeAssignCourier = async (
    ...
    try {
      ...
      toast({ title: 'Courier Assigned', description: `${courierData.courier_name} assigned successfully`, variant: 'success' });
      refreshAll();
    } catch (e) {
      toast({ title: 'Error', description: e instanceof Error ? e.message : 'Failed to assign courier', variant: 'destructive' });
    }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Order Detail Modal ──────────────────────────────────────────────────── */}
      <Dialog open={isModalOpen} onOpenChange={(open) => { setIsModalOpen(open); if (!open) setInlineConfirm(null); }}>
        <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 gap-0">
          <DialogHeader className="px-6 pt-6 pb-2 flex-shrink-0">
            <DialogTitle className="text-base font-semibold">
              Order Details{' '}
          ...
          {selectedOrder && (() => {
            ...
            return (
              <div className="space-y-6 overflow-y-auto flex-1 px-6 pb-4 pt-2">
                ...
                {/* Workflow actions — inline confirm to avoid z-index conflict with external dialog */}
                <div className="space-y-2">
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Actions</h3>

                  {inlineConfirm && inlineConfirm.orderId === selectedOrder.order_id ? (
                    <div className="flex items-center justify-between gap-3 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                      <p className="text-sm text-amber-800 font-medium">
                        Confirm: {inlineConfirm.label}?
                      </p>
                      <div className="flex gap-2 flex-shrink-0">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            const { action, label, orderId, courierData } = inlineConfirm;
                            setInlineConfirm(null);
                            if (action === 'assign-courier' && courierData) {
                              await executeAssignCourier(orderId, courierData);
                            } else {
                              await handleOrderAction(orderId, action as any, label, true);
                            }
                          }}
                        >
                          {inlineConfirm.label}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedOrder.status === 'processing' && (
                        <Button variant="outline" size="sm" className="gap-2"
                          onClick={() => setInlineConfirm({ action: 'pack', label: 'Pack Order', orderId: selectedOrder.order_id })}>
                          <Package className="h-4 w-4" /> Pack Order
                        </Button>
                      )}
                      {selectedOrder.status === 'ready_for_pickup' && (
                        <>
                          <Button variant="outline" size="sm" className="gap-2"
                            onClick={() => setInlineConfirm({ action: 'pickup', label: 'Mark Picked Up', orderId: selectedOrder.order_id })}>
                            <UserCheck className="h-4 w-4" /> Mark Picked Up
                          </Button>
                          <Button variant="outline" size="sm" className="gap-2"
                            onClick={() => {
                              ...
                              if (courier_name && courier_tracking_number) {
                                setInlineConfirm({
                                  ...
                                  courierData: { courier_name, courier_tracking_number, delivery_notes },
                                });
                              }
                            }}>
                            <Truck className="h-4 w-4" /> Assign Courier
                      ...
                      {selectedOrder.status === 'picked_up_by_courier' && (
                        <Button variant="outline" size="sm" className="gap-2"
                          onClick={() => setInlineConfirm({ action: 'out-for-delivery', label: 'Out for Delivery', orderId: selectedOrder.order_id })}>
                          <Truck className="h-4 w-4" /> Out for Delivery
                        </Button>
                      )}
                      {selectedOrder.status === 'out_for_delivery' && (
                        <Button variant="outline" size="sm"
                          className="gap-2 text-emerald-700 border-emerald-300 hover:bg-emerald-50"
                          onClick={() => setInlineConfirm({ action: 'deliver', label: 'Mark Delivered', orderId: selectedOrder.order_id })}>
                          <CheckCircle className="h-4 w-4" /> Mark Delivered
                        </Button>
                      )}
                      {!['processing', 'ready_for_pickup', 'picked_up_by_courier', 'out_for_delivery'].includes(selectedOrder.status) && (
